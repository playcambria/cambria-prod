...
export class SocketManager {
    ...
    async getOrCreateRoom(roomId: string): Promise<GameRoom> {
        ...
        if (roomPromise === undefined) {
            roomPromise = GameRoom.create(
                ...
                (chatEventMessage: ChatEventMessage, listener: ZoneSocket) => this.onChat(chatEventMessage, listener),
                (pfpUpdateMessage: PfpUpdateMessage, listener: ZoneSocket) =>
                    this.onPfpUpdate(pfpUpdateMessage, listener),
    ...
    private onZoneEnter(thing: Movable, fromZone: Zone | null, listener: ZoneSocket) {
        if (thing instanceof User) {
            ...
            if (thing.walletAddress) {
                userJoinedZoneMessage.setWalletaddress(thing.walletAddress);
            }
    ...
    private onChat(chatEventMessage: ChatEventMessage, client: ZoneSocket) {
        const subMessage = new SubToPusherMessage();
        subMessage.setChateventmessage(chatEventMessage);

        emitZoneMessage(subMessage, client);
    }

    private onPfpUpdate(pfpUpdateMessage: PfpUpdateMessage, client: ZoneSocket) {
        const subMessage = new SubToPusherMessage();
        subMessage.setPfpupdatemessage(pfpUpdateMessage);
    ...
    /**
     * Computes a unique user/password for the TURN server, using a shared secret between the Cambria API server
    ...
    public async addZoneListener(call: ZoneSocket, roomId: string, x: number, y: number): Promise<void> {
        ...
        for (const thing of things) {
            if (thing instanceof User) {
                ...
                if (thing.walletAddress) {
                    userJoinedMessage.setWalletaddress(thing.walletAddress);
                }
    ...
    handleChatEventMessage(room: GameRoom, user: User, chatPromptMessage: ChatPromptMessage) {
        const chatEventMessage = new ChatEventMessage();
        chatEventMessage.setText(chatPromptMessage.getText());
        chatEventMessage.setActoruserid(user.id);
        room.emitChatEvent(user, chatEventMessage);
    }

    handleTradeMessage(room: GameRoom, user: User, message: TradeMessage) {

        interface TradeWithdrawPayload {
            recipients: number[];
        }

        const payloadString = message.getEventpayloadjson();
        let payloadJSON: TradeWithdrawPayload = { recipients: [] };

        if (payloadString && payloadString.length > 0) {
            try {
                payloadJSON = JSON.parse(payloadString) as TradeWithdrawPayload;
            } catch (err) {
                console.error(err);
            }
        }
        ...
        switch (message.getEventtype()) {
            ...
            case "fillOrder":
            case "tradeComplete":
                clientMessage = new ServerToClientMessage();
                clientMessage.setTrademessage(message);
                room.getUserById(message.getTarget())?.socket.write(clientMessage);
                break;
            case "withdrawRequest":
                if (payloadJSON) {
                    const recipients = payloadJSON.recipients;
                    for (const _recipient of recipients) {
                        clientMessage = new ServerToClientMessage();
                        clientMessage.setTrademessage(message);
                        const targetUser = room.getUserById(_recipient);
                        targetUser?.socket.write(clientMessage);
                        console.log("Broadcasting message to recipient:", _recipient);
                    }
                }
                break;
            case "whackEvent":
                console.log("whack request received!")
                clientMessage = new ServerToClientMessage();
                clientMessage.setTrademessage(message);
                room.getUserById(message.getTarget())?.socket.write(clientMessage);
                break;
    ...
    handlePfpUpdateMessage(room: GameRoom, user: User, message: PfpUpdateMessage) {
        const clientMessage = new ServerToClientMessage();
        clientMessage.setPfpupdatemessage(message);
        message.setUserid(user.id);
        room.emitPfpUpdateEvent(user, message);
    }
