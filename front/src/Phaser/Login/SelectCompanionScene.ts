...
import { getCompanionPet, setCompanionPet } from "../../Supabase/user";
...
export class SelectCompanionScene extends ResizableScene {
    ...
    create() {
        ...
        getCompanionPet(walletAddress).then((companionPet) => {
            if (companionPet.data?.companion_pet) {
                gameManager.setCompanion(companionPet.data?.companion_pet);
                const companionIndex = this.companionModels.findIndex(
                    (companion) => companion.name === companionPet.data?.companion_pet
                );
    ...
    public selectCompanion(): void {
        setCompanionPet(this.companionModels[this.currentCompanion].name);
