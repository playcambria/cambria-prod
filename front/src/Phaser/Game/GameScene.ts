...
import { hasMovedEventName, hasTakenActionEventName, Player, requestEmoteEventName } from "../Player/Player";
...
import type { HasPlayerTakenActionEvent } from "../../Api/Events/HasPlayerTakenActionEvent";
...
import { chatMessageStore } from "../../Stores/ChatStore";
import {
    gameManagerReadyStore,
...
import {
    sendRequestAcceptanceStore,
    outgoingRequestWithdrawStore,
    activeTradeStore,
    showTradeScreen,
...
import { createUser, getUserNfts, getPfpUrl } from "../../Supabase/user";
import { getFriends } from "../../Supabase/friends";
import { PfpUpdateManager } from "./PfpUpdateManager";
import { Nft } from "../../Components/Navigation/Inventory/types";
import { NpcPlayer, NpcType } from "../Entity/NpcPlayer";
import { getUserQuests } from "../../Supabase/user-quests";
import { QuestStatus, shownQuestStore, UserQuest, userQuestsStore } from "../../Stores/QuestStore";
import { bankTabsStore } from "../../Stores/BankStore";
...
interface UserActionEventInterface {
    ...
    event: MessageUserActionInterface;
}
...
export class GameScene extends DirtyScene {
    ...
    BankNpcPlayerA!: NpcPlayer;
    BankNpcPlayerB!: NpcPlayer;
    AnnouncerNpcPlayer!: NpcPlayer;
    ...
    private chatUnsubscriber!: Unsubscriber;
    private tradeRequestAcceptanceUnsubscriber!: Unsubscriber;
    private outgoingRequestWithdrawStoreUnsubscriber!: Unsubscriber;
    private emoteMenuUnsubscriber!: Unsubscriber;
    private tradeScreenUnsubscriber!: Unsubscriber;
    ...
    lastActionSent: HasPlayerTakenActionEvent = {
        action: "none",
        effect: "none",
        duration: 0,
        targetId: -1
    ...
    private pfpUpdateManager!: PfpUpdateManager;
    ...
    preload(): void {
        ...
        // this.listenToIframeEvents(); // DISABLED FOR NOW

        this.load.atlas('effects', '/resources/effects/stun.png', '/resources/effects/stun.json');
        this.load.atlas('funny-items', '/resources/misc/items.png', '/resources/misc/items.json');
        ...
        // preload NFT image
        this.load.image("pfpPlaceholder", "/resources/placeholders/pfp.png"); // TODO: make this dynamic
        ...
        // sounds
        this.load.audio("whack", "/resources/audio/effects/whack.wav");

        // Load FX
        this.load.audio("audio-webrtc-in", "/resources/objects/fx/webrtc-in.mp3");
        this.load.audio("audio-webrtc-out", "/resources/objects/fx/webrtc-out.mp3");
        this.load.audio("audio-report-message", "/resources/objects/fx/report-message.mp3");
        this.load.audio("hover", "/resources/objects/fx/hover.mp3");
        this.load.audio("achievement", "/resources/objects/fx/Achievement.mp3");
        this.load.audio("sparkle-confirm", "/resources/objects/fx/Collect Item.mp3");
        this.load.audio("sparkle-confirm-long", "/resources/objects/fx/Magic Prize.mp3");
        this.load.audio("confirm-button", "/resources/objects/fx/Confirm Button.mp3");
        this.load.audio("confirm-button-short", "/resources/objects/fx/Game Coin.mp3");
        this.load.audio("notification", "/resources/objects/fx/Earn Points.mp3");
        this.load.audio("unlock", "/resources/objects/fx/Secret Area Unlock 1.mp3");
        this.load.audio("fail", "/resources/objects/fx/Soft Fail.mp3");
        this.load.audio("victory", "/resources/objects/fx/Victory.mp3")
        this.load.audio("button-feedback", "/resources/objects/fx/feedback.mp3");
        this.load.audio("message", "/resources/objects/fx/Message.mp3");

        // Game FX
        this.load.audio("bright-popup", "/resources/objects/menu_fx/Bright Popup.mp3");
        this.load.audio("button-press", "/resources/objects/menu_fx/Button Press.mp3");
        this.load.audio("chat-alert", "/resources/objects/menu_fx/Chat Alert.mp3");
        this.load.audio("chat-message", "/resources/objects/menu_fx/Chat Message.mp3");
        this.load.audio("chat-message-reversed", "/resources/objects/menu_fx/Chat Message Reversed.mp3");
        this.load.audio("chat-notification", "/resources/objects/menu_fx/Chat Notification.mp3");
        this.load.audio("menu-alert", "/resources/objects/menu_fx/Menu Alert.mp3");
        this.load.audio("menu-button", "/resources/objects/menu_fx/Menu Button.mp3");
        this.load.audio("menu-cancel", "/resources/objects/menu_fx/Menu Cancel.mp3");
        this.load.audio("menu-click", "/resources/objects/menu_fx/Menu Click.mp3");
        this.load.audio("menu-complete", "/resources/objects/menu_fx/Menu Complete.mp3");
        this.load.audio("menu-confirm", "/resources/objects/menu_fx/Menu Confirm.mp3");
        this.load.audio("menu-confirmation", "/resources/objects/menu_fx/Menu Confirmation.mp3");
        this.load.audio("menu-error", "/resources/objects/menu_fx/Menu Error.mp3");
        this.load.audio("menu-message", "/resources/objects/menu_fx/Menu Message.mp3");
        this.load.audio("menu-notification", "/resources/objects/menu_fx/Menu Notification.mp3");
        this.load.audio("menu-popup", "/resources/objects/menu_fx/Menu Pop Up.mp3");
        this.load.audio("menu-success", "/resources/objects/menu_fx/Menu Success.mp3");
        ...
        this.load.on(FILE_LOAD_ERROR, (file: { src: string }) => {
            ...
            //if SpriteSheetFile (avatar file) don't display error and give an access for user
    ...
    create(): void {
        ...
        this.anims.create({
            key: 'chicken-slap',
            repeat: 1,
            frames: this.anims.generateFrameNames('funny-items', {
                prefix: 'rubber_chicken/anim_',
                suffix: '.png',
                start: 0,
                end: 4,
                zeroPad: 1
        ...
        // Add NPC Players
        this.createBankNpcPlayer();
        this.createAnnouncerNpcPlayer();
        ...
        Promise.all([
            ...
            .then(() => {
                this.scene.wake();
                this.loader.removeLoader();
                gameManagerReadyStore.set(true);
            })
            .catch((e) =>
                console.error("Some scripts failed to load ot the connection failed to establish to Cambria server", e)
            );
    }
    ...
    private async connect(): Promise<void> {
        ...
        console.log("CONNECTING TO ROOM WITH WALLET:", this.walletAddress);
        try {
            // Create user in DB if it doesn't exist
            createUser(this.walletAddress).catch((err) => {
                console.error(err);
            });
            // Fetch friends of the user
            await getFriends(this.walletAddress)
                .then(({ friends }) => {
                    console.log({ friends });
                    if (friends) {
                        friendsListStore.set(
                            friends.reduce((accumulator, value) => {
            ...
            // Fetch quests of the user
            await getUserQuests(this.walletAddress)
                .then((quests) => {
                    console.log({ quests });
                    if (quests) {
                        userQuestsStore.set(quests);
                    }
                })
                .catch((err) => {
                    console.error(err);
            ...
            // Fetch bank tabs
            await getUserNfts(this.walletAddress)
                .then(({ bankTabIds, inventoryNftIds }) => {
                    if (bankTabIds) {
                        bankTabsStore.set(bankTabIds);
                    }
                    if (inventoryNftIds) {
                        inventoryNFTIdsState.set(inventoryNftIds);
                    }
                })
                .catch((err: any) => {
                    console.error(err);
            ...
            // const shownQuest = get(userQuestsStore).find((quest: UserQuest)=> quest.status ===QuestStatus.NOT_STARTED);

            // shownQuestStore.set(shownQuest);
        } catch (err) {
        ...
        connectionManager
            ...
            .then((onConnect: OnConnectInterface) => {
                ...
                this.CurrentPlayer.on(hasTakenActionEventName, (event: HasPlayerTakenActionEvent) => {
                    this.handleCurrentPlayerHasActionEvent(event);
                });
                ...
                this.chatManager = new ChatManager(this, this.connection);
                this.pfpUpdateManager = new PfpUpdateManager(this, this.connection);
                this.tradeManager = new TradeManager(this, this.connection);
    ...
    private subscribeToStores(): void {
        if (
            ...
            this.tradeScreenUnsubscriber != undefined ||
        ...
        this.chatUnsubscriber = chatMessageStore.subscribe((chatText) => {
            if (chatText) {
                this.connection?.emitChatEvent(chatText);
                chatMessageStore.set("");
            }
        });

        this.tradeRequestAcceptanceUnsubscriber = sendRequestAcceptanceStore.subscribe((target: number) => {
            if (target > 0) {
                this.connection?.emitTradeRequestAcceptanceEvent(target);
                sendRequestAcceptanceStore.set(0); // reset this flag
            }
        });

        this.outgoingRequestWithdrawStoreUnsubscriber = outgoingRequestWithdrawStore.subscribe(
            (toWithdraw: number[]) => {
                if (toWithdraw && toWithdraw.length > 0) {
                    this.connection?.emitOutgoingRequestWithdrawEvent(toWithdraw);
                    outgoingRequestWithdrawStore.set([]); // reset this queue
                }
            }
        );
        ...
        this.tradeScreenUnsubscriber = activeTradeStore.subscribe((activeTrade) => {
            if (activeTrade && showTradeScreen) {
                this.userInputManager.disableControls();
            } else {
                this.userInputManager.restoreControls();
            }
...
${escapedMessage}
    ...
    public declineTrade(target: number) {
        return this.connection?.emitTradeDeclineEvent(target);
    }

    public signedOrder(target: number, signedOrder: SignedOrder) {
        return this.connection?.emitSignedOrderEvent(target, signedOrder);
    }

    public fillOrder(target: number) {
        return this.connection?.emitFillOrderEvent(target);
    }

    public completeTrade(target: number) {
        return this.connection?.emitTradeCompleteEvent(target);
    }

    public offerUpdate(target: number, myOffer: Array<Token | Nft>) {
        return this.connection?.emitOfferUpdateEvent(target, myOffer);
    }

    public offerConfirm(target: number) {
        return this.connection?.emitOfferConfirmEvent(target);
    }

    public requestTrade(target: number) {
        return this.connection?.emitTradeRequestEvent(target);
    }

    public requestWhack(target: number) {
        return this.connection?.emitWhackRequestEvent(target);
    }
    ...
    public cleanupClosingScene(): void {
        try {
            ...
            this.pfpUpdateManager?.destroy();
            this.cameraManager?.destroy();
            this.peerStoreUnsubscriber?.();
            this.emoteUnsubscriber?.();
            this.chatUnsubscriber?.();
            this.tradeRequestAcceptanceUnsubscriber?.();
            this.outgoingRequestWithdrawStoreUnsubscriber?.();
            this.emoteMenuUnsubscriber?.();
    ...
    public moveToRemotePlayer(remotePlayerId: number, whack: boolean = false): void {
        const target = this.MapPlayersByKey.get(remotePlayerId);
        if (target) {
            const targetPosition = target.getPosition();
            const startTile = this.gameMap.getTileIndexAt(this.CurrentPlayer.x, this.CurrentPlayer.y);
            const targetTile = this.gameMap.getTileIndexAt(targetPosition.x, targetPosition.y);

            this.getPathfindingManager()
                .findPath(startTile, targetTile, true, true)
                .then((path) => {
                    // Remove last step, to avoid walking straight into the target player
                    console.log("AT", startTile, targetTile, this.CurrentPlayer, path)
                    path.pop();
                    // Start pathing towards target
                    this.CurrentPlayer.setPathToFollow(path)
                        .then(() => {
                            if (whack) {
                                this.CurrentPlayer.chickenSlap();
                                this.requestWhack(remotePlayerId);
                                this.playSound("whack");
                                this.MapPlayersByKey.get(remotePlayerId)?.stun(1000);
                            }
                        }).catch((err) => {
                            console.error(err);
                        });
    ...
    private handleCurrentPlayerHasMovedEvent(event: HasPlayerMovedEvent): void {
        ...
        this.activatablesManager.updateActivatableObjectsDistances([...this.actionableItems.values()]);
    ...
    private createCurrentPlayer() {
        ...
        try {
            this.CurrentPlayer = new Player(
                ...
                false,
                this.companion,
                this.companion ? lazyLoadCompanionResource(this.load, this.companion) : undefined
            );

            console.log("CURRENTAPLAYER", this.CurrentPlayer);
            getPfpUrl(this.walletAddress || "").then(({ data }) => {
                console.log({ data });
                if (data && data.pfp_url) {
                    let pfp_url = JSON.parse(data.pfp_url)?.iconUrl;
                    this.CurrentPlayer.setPfp(pfp_url);
                }
            });
            // Handle input events (left / right click, tap) on self (player)
    ...
    private createBankNpcPlayer() {
        const bankerA = lazyLoadPlayerCharacterTextures(this.superLoad, [
            {
                "id": "body3",
            ...
            {
                "id": "clothes18",
                "img": "resources/customisation/character_clothes/character_clothes17.png"
            },
            {
                "id": "hat1",
        ...
        const bankerB = lazyLoadPlayerCharacterTextures(this.superLoad, [
            {
                "id": "body5",
        ...
        this.BankNpcPlayerA = new NpcPlayer(
            this,
            this.startPositionCalculator.startPosition.x + 200,
            this.startPositionCalculator.startPosition.y + 50,
            "Arthur",
            bankerA,
            PlayerAnimationDirections.Down,
            false,
        ...
        this.BankNpcPlayerB = new NpcPlayer(
            this,
            this.startPositionCalculator.startPosition.x + 360,
            this.startPositionCalculator.startPosition.y + 50,
            "Kristen",
            bankerB,
            PlayerAnimationDirections.Down,
    ...
    private createAnnouncerNpcPlayer() {
        const announcer = lazyLoadPlayerCharacterTextures(this.superLoad, [
            {
                "id": "body2",
            ...
            {
                "id": "clothes63",
                "img": "resources/customisation/character_clothes/character_clothes62.png"
            },
            {
                "id": "hat1",
                "img": "resources/customisation/character_hats/character_hats1.png"
        ...
        this.AnnouncerNpcPlayer = new NpcPlayer(
            this,
            this.startPositionCalculator.startPosition.x + 280,
            this.startPositionCalculator.startPosition.y + 450,
            "Announcer",
            announcer,
            PlayerAnimationDirections.Right,
            false,
            NpcType.Announcer,
            true
        );
    ...
    public update(time: number, delta: number): void {
        ...
        // Process NPC Wander
        this.AnnouncerNpcPlayer.processWander();
        this.AnnouncerNpcPlayer.moveNpc(delta);
        ...
        while (this.pendingEvents.length !== 0) {
            ...
            switch (event.type) {
                ...
                case "UserActionEvent": {
                    this.doUpdatePlayerAction(event.event);
                    const remotePlayer = this.MapPlayersByKey.get(event.event.userId);
                    break;
                }
    ...
    private doAddPlayer(addPlayerData: AddPlayerInterface): void {
        ...
        const player = new RemotePlayer(
            ...
            addPlayerData.walletAddress,
            addPlayerData.companion,
            addPlayerData.companion ? lazyLoadCompanionResource(this.load, addPlayerData.companion) : undefined
        );
    ...
    private doUpdatePlayerAction(message: MessageUserActionInterface): void {
        const player: RemotePlayer | undefined = this.MapPlayersByKey.get(message.userId);

        if (player == undefined) {
            console.error("Can not whack player: " + message.userId + " because player is not found.");
            return;
        }

        console.log("We need to do the phaser stuff for whacking.");
    ...
    private bannedUser() {
        ...
        this.scene.start(ErrorSceneName, {
            title: "Banned",
            subTitle: "You were banned from Cambria",
            message: "If you want more information, you may contact us at: hello@cambria.gg",
        });
    }
    ...
    private showWorldFullError(message: string | null): void {
        ...
        if (message == undefined) {
            ...
            this.scene.start(ErrorSceneName, {
                ...
                message:
                    "If you want more information, you may contact administrator or contact us at: hello@cambria.gg",
            });
        }
    }
