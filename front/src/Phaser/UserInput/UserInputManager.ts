...
export class UserInputManager {
    ...
    private chatInputFocus: boolean;
    private menuInputFocus: boolean;
    private textInputFocus: boolean;
    ...
    constructor(scene: Phaser.Scene, userInputHandler: UserInputHandlerInterface) {
        ...
        // Subscribe to chatInputFocus, menuInputFocus, and textInputPrompt to correctly set flags to determine whether we auto-force focus to chat message input
        chatInputFocusStore.subscribe((isFocused) => {
            this.chatInputFocus = isFocused;
        });
        menuInputFocusStore.subscribe((isFocused) => {
            this.menuInputFocus = isFocused;
        });
        textInputPromptStore.subscribe((textInputPrompt) => {
    ...
    private bindInputEventHandlers() {
        ...
        this.scene.input.keyboard.on("keydown", (event: Event) => {
            if (this.isInputDisabled) {
                return;
            }
            if (event.keyCode === 32 || (event.keyCode >= 48 && event.keyCode < 90)) {
                // For wqerty type keyboard input, auto-focus to chat if we aren't in some other text input window
                if (this.chatInputFocus === false && this.menuInputFocus === false && this.textInputFocus === false) {
                    gameKeyboardStore.update((current) => {
                        if (current) {
