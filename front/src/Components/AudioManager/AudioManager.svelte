<script lang="ts">
    ...
    onMount(() => {
        ...
        unsubscriberFileStore = audioManagerFileStore.subscribe((src: string) => {
            ...
            HTMLAudioPlayer.loop = true; // get(audioManagerVolumeStore).loop;
        ...
        unsubscriberVolumeStore = audioManagerVolumeStore.subscribe((audioManager: audioManagerVolume) => {
            ...
            HTMLAudioPlayer.loop = true; // audioManager.loop;
            updateVolumeUI();
        });
    });
...
<div class:hidden={!showAudioControls} on:mouseenter={() => active = true} on:mouseleave={() => { active = false}}>
