            ...
            (message: MessageEvent<unknown>) => {
                ...
                if (foundSrc === undefined || iframe === undefined) {
                    if (lookingLikeEvent.success) {
                        console.warn(
                            "It seems an iFrame is trying to communicate with Cambria but was not explicitly granted the permission to do so. " +
                                "If you are looking to use the Cambria Scripting API inside an iFrame, you should allow the " +
                                'iFrame to communicate with Cambria by using the "openWebsiteAllowApi" property in your map (or passing "true" as a second' +
