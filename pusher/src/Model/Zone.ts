...
export interface ZoneEventListener {
    ...
    onChat(emoteMessage: ChatEventMessage, listener: ExSocketInterface): void;
    onPfpUpdate(pfpUpdateMessage: PfpUpdateMessage, listener: ExSocketInterface): void;
...
export class UserDescriptor {
    ...
    public toUserJoinedMessage(): UserJoinedMessage {
        ...
        if (this.walletAddress) {
            userJoinedMessage.setWalletaddress(this.walletAddress);
        }
...
export class Zone {
    ...
    public async init(): Promise<void> {
        ...
        this.backConnection.on("data", (batch: BatchToPusherMessage) => {
            for (const message of batch.getPayloadList()) {
                ...
                } else if (message.hasChateventmessage()) {
                    const chatEventMessage = message.getChateventmessage() as ChatEventMessage;
                    this.notifyChat(chatEventMessage);
                } else if (message.hasPfpupdatemessage()) {
                    const pfpUpdateMessage = message.getPfpupdatemessage() as PfpUpdateMessage;
                    this.notifyPfpUpdate(pfpUpdateMessage);
    ...
    private notifyChat(chatMessage: ChatEventMessage): void {
        for (const listener of this.listeners) {
            if (listener.userId === chatMessage.getActoruserid()) {
                continue;
            }
            this.socketListener.onChat(chatMessage, listener);
        }
    }

    private notifyPfpUpdate(pfpUpdateMessage: PfpUpdateMessage): void {
        console.log({ pfpUpdateMessage });
        for (const listener of this.listeners) {
            if (listener.userId === pfpUpdateMessage.getUserid()) {
                continue;
            }
            this.socketListener.onPfpUpdate(pfpUpdateMessage, listener);
        }
    }
