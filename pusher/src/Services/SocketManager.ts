...
export class SocketManager implements ZoneEventListener {
    ...
    async handleJoinRoom(client: ExSocketInterface): Promise<void> {
        ...
        try {
            ...
            joinRoomMessage.setWalletaddress(client.walletAddress);
    ...
    handleTrade(client: ExSocketInterface, message: TradeMessage): void {
        const pusherToBackMessage = new PusherToBackMessage();
        pusherToBackMessage.setTrademessage(message);
        client.backConnection.write(pusherToBackMessage);
    }
    ...
    onChat(chatMessage: ChatEventMessage, listener: ExSocketInterface): void {
        const subMessage = new SubMessage();
        subMessage.setChateventmessage(chatMessage);
    ...
    handleChatPromptMessage(client: ExSocketInterface, chatPromptmessage: ChatPromptMessage): void {
        console.log("TRACE2");
        const pusherToBackMessage = new PusherToBackMessage();
        pusherToBackMessage.setChatpromptmessage(chatPromptmessage);

        client.backConnection.write(pusherToBackMessage);
    }

    handlePfpUpdate(client: ExSocketInterface, pfpUpdatemessage: PfpUpdateMessage): void {
        console.log("Pfp Update in pusher socket manager");
        const pusherToBackMessage = new PusherToBackMessage();
        pusherToBackMessage.setPfpupdatemessage(pfpUpdatemessage);
