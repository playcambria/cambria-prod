...
import { UserMovesMessage, TradeMessage } from "../Messages/generated/messages_pb";
...
export class IoSocketController {
    ...
    ioConnection(): void {
        this.app.ws("/room", {
            ...
            upgrade: (res, req, context) => {
                (async (): Promise<void> => {
                    ...
                    try {
                        ...
                        console.log("SOCKET QUERY: WALLET", walletAddress);
                            ...
                            return res.upgrade(
                                {
                                    ...
                                    error: {
                                        ...
                                        code: "NEW_VERSION",
                                        details: "A new version of Cambria is available. Please refresh your window",
                                        canRetryManual: true,
                                        buttonTitle: "Refresh",
                                        timeToRetry: 999999,
                                    },
                        ...
                        let memberWalletaddress: string | null = null;
                        ...
                        if (ADMIN_API_URL) {
                            try {
                                ...
                                memberWalletaddress = userData.walletAddress;
                        ...
                        res.upgrade(
                            {
                                ...
                                walletAddress: walletAddress, // optionally, if admin api, use memberWalletaddress
            ...
            message: (ws, arrayBuffer): void => {
                ...
                } else if (message.hasChatpromptmessage()) {
                    console.log("TRACE1");
                    socketManager.handleChatPromptMessage(client, message.getChatpromptmessage() as ChatPromptMessage);
                } else if (message.hasPfpupdatemessage()) {
                    console.log("PFP UPDATE");
                    socketManager.handlePfpUpdate(client, message.getPfpupdatemessage() as PfpUpdateMessage);
                } else if (message.hasTrademessage()) {
                    socketManager.handleTrade(client, message.getTrademessage() as TradeMessage);
    ...
    private initClient(ws: any): ExSocketInterface {
        ...
        client.walletAddress = ws.walletAddress;
